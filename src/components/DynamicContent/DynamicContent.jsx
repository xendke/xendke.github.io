...
const DynamicContent = ({ refs, handleSectionChange }) => {
    ...
    useEffect(() => {
        if(dynamicContentRef.current) {
            const containerTop = dynamicContentRef.current.getBoundingClientRect().y;
            let timeout;
            ...
            const onScroll = () => {
                ...
                timeout = window.requestAnimationFrame(() => {
                    for(let section in refs) {
                        const currentSectionTop = refs[section].current.getBoundingClientRect().y;
                        if(Math.abs(Math.floor(currentSectionTop) - Math.floor(containerTop)) < 100) {
